...
(define (py-read)
  ...
  (define (get-indent-and-tokens)
    ...
    (if (eq? (peek-char) #\space)
        (begin (set! num-spaces (+ 1 num-spaces))
               (read-char)
               (get-indent-and-tokens))
        (cons num-spaces (get-tokens '()))))
  ...
  (define dot-seen #f)
  (define (get-num num-so-far dot-seen)
    ...
    (let ((char (peek-char)))
      (if (eof-object? char)
      	  (begin (read-char) num-so-far)
      	  (if (char-numeric? char) 
      	      (get-num (word num-so-far (char->symbol (read-char))) dot-seen)
              (if (and (not dot-seen) (eq? #\. char))
                  (begin (set! dot-seen #t)
                    (get-num (word num-so-far (char->symbol (read-char))) dot-seen))
                  num-so-far)))))
  ...
  (define (get-string type)
   ...
   (define (get-all-string so-far)
     (if (eq? so-far type)
         nil
         (if (char-newline? so-far)
             (read-error "string not finished")
             (cons so-far (get-all-string (read-char))))))

    (let ((next (peek-char)))
      (cond ((eq? next type) (begin (read-char) nil))
            ((char-newline? next) (read-error "string not finished"))
            (else (get-all-string (read-char))))))
          
  (define (ignore-comment)
    (let ((next (read-char)))
      (if (or (eof-object? next) (char-newline? next))
