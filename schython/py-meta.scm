  ...
  (if (ask line-obj 'empty?)
	...
	(cond ((infix-op? token) ;; arithmetic infix operators
         ...
         (if (ask val 'true?)
	      ...
	      ((or? token)
	       (if (ask val 'true?)
             (begin (eat-tokens line-obj) val)
             (py-eval line-obj env)))

	      ;; test for membership
	      ((in? token)
         (ask (py-eval line-obj env) '__contains__ val))

	      ((not? token)
          (if (in? (ask line-obj 'next))
              (negate-bool (ask (py-eval line-obj env) '__contains__ val))
              (py-error "'not' not followed by in")))
...
(define (value-grabber line-obj env close-token)
(let ((token (ask line-obj 'next)))
    ...
    (else                         
     ...
     (let ((key (py-eval line-obj env)))     
       (cond ((colon? (ask line-obj 'peek))
     (begin (ask line-obj 'next) 
      (let ((michaelsarus (py-eval line-obj env)))  
        (cons (cons key michaelsarus)  
          (if (comma? (ask line-obj 'peek))
            (collect-key-value line-obj env close-token)
            (if (eq? (ask line-obj 'peek) close-token)
              (begin (ask line-obj 'next) '())
              (py-error "Dictonary not formatted correctly")))))))
     (py-error "Dictonary not formatted correctly")))))))

(define (collect-key-value line-obj env close-token)
  (value-grabber line-obj env close-token))
  ; (if (eq? (ask line-obj 'peek) close-token)
  ;     nil
  ;     (let ((key (ask line-obj 'next)))
  ;       (ask line-obj 'next) ;eat ":"
  ;       (let ((value (ask line-obj 'next)))
  ;         ; (ask line-obj 'next) ;eat ","
  ;         (list (cons key value))
  ;       )
  ;     )
...
(define (collect-pred line-obj env)
    (let ((token (ask line-obj 'next)))
      (if (colon? token)
          nil
          (cons token (collect-pred line-obj env)))))

(define (add-ind line-obj env)
  (let ((indentation (ask line-obj 'indentation))
    (pred-list (collect-pred line-obj env)))
  (cons indentation pred-list)))


(define (make-if-block line-obj env)
  (let ((pred (add-ind line-obj env))
...
(define (eval-if-block block env)
  (let ((pred (if-block-pred block))
    ...
    (let ((should-eval-else else-clause))
      (define (loop)
...
(define (elif-block-else block)  
 (let ((body (cadddr while-block)))
  (cdr (split-block body))))

(define (elif->if block)
  (cond ((null? block) nil)
        ((eq? block '*ELIF-BLOCK*) '*IF-BLOCK*)
        (else (cons (car block) (elif->if (cdr block))))))
...
(define (eval-def-block block env)
  (let ((proc (make-py-proc (def-block-name block) ;define at py-primitives l563
...
(define (make-for-block line-obj env)
  ...
  ;; Start making for block as cleverly stated in variable name
  ;; Ignore/eat the for token
  ;; Evaluate the rest
  (let 
    ((var (ask line-obj 'next)) (collection (begin (ask line-obj 'next)
      (collect-pred line-obj env))) (body (read-block (ask line-obj 'indentation) 
      env)))
    ;;))
  (list '*BLOCK* '*FOR-BLOCK* (cons var collection) body)))

(define (for-block-var block)
  ;(py-error "TodoError: Person A, Question 7"))
  ;;(*A7)
  ;; Return the loop in the for
...
(define (for-block-body block)
  ;;(py-error "TodoError: Person A, Question 7"))
  ;;(*A7)
  ;; Returns lines with indentation
  ...
  ;; the body of the "FOR-block"
  (car (split-block (cadddr block))))

(define (for-block-else block)
  ;;(py-error "TodoError: Person A, Question 7"))
  ;;(*A7)
  ;; Returns else block if one exists
...
(define (eval-for-block block env)
  ;;(py-error "TodoError: Person A, Question 7"))
  ;;(*A7)
  ;; Evaluates the input in the for
  ;; The EVAL-FOR-BLOCK procedure takes in a "FOR-block" and 
  ;; the currentenvironment as arguments.
  (let 
    ((collection-obj (py-eval (instantiate line-obj '*DUMMY-INDENT* (for-block-collection block))
       env))
  ;; (for-block-var block))
  ...
  (else-clause (for-block-else block)))
    ...
    ;; Abstraction is my new best friend.
    ;; Helper for eval-for-block
    ;; inspired by question 9.
    (define (abstraction block env)
      (if (eq? result '*BREAK)
        ((set! should-eval-if #f) *NONE*)
        (if (not (eq? should-eval-if #f))
          (eval-item (make-line-obj else-clause) env)
          *NONE*)))
    ;;(define (helper)
    ;;  (if (eq? result '*BREAK)
    ;;    ((set! should-eval-if #t) *NONE*)
    ;;    (if (not (eq? should-eval-if #t))
    ;;     (eval-item (make-line-obj else-clause) env))))
    (abstraction block env))))
