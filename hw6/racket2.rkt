...
(define (eval-1 exp)
  ...
  (cond ((constant? exp) exp)
    ...
    ; ((symbol? exp) 'symbollll)  ; use underlying Racket's EVAL
    ((quote-exp? exp) (cadr exp))
    ; ((quote-exp? exp) 'quoooooot)
    ((if-exp? exp)
     (if (eval-1 (cadr exp))
         (eval-1 (caddr exp))
         (eval-1 (cadddr exp))))
    ; ((lambda-exp? exp) exp)
    ((lambda-exp? exp) (eval exp))
    ; ((lambda-exp? exp) 'lambdaaaaaaa)
    ; ((pair? exp) (apply-1 (eval-1 (car exp))      ; eval the operator
    ;               (map eval-1 (cdr exp))))
    ; ((pair? exp) #t) 
    ; ((pair? exp) (car exp))              ;'map 'let
    ; ((pair? exp) (cdr exp))              ;'(((x 1) (y 2)) (+ x y))
    ; ((pair? exp) (first (cdr exp)))              ;'((x 1) (y 2))
    ; ((pair? exp) (length (cdr exp)))              ;'((x 1) (y 2))                '((lambda (x) (first x)) '(the rain in spain))
    ; ((pair? exp) (eval-1 (first (cdr exp))))              ;2    '((lambda (x) (first x)) '(the rain in spain))
                                         ;'(first '(the rain in spain))
    ((pair? exp) (map eval-1 (cdr exp))) ;'((lambda (x) (first x)) '(the rain in spain))
                                        ;'(#<procedure:first> (the rain in spain)) for (map first '(the rain in spain))
    ; ((pair? exp) (eval-1 (car exp)))     ;#<procedure:let>
...
(define (apply-1 proc args)
  ...
  ;    (apply proc args))       ;这个例子 (map first '(the rain in spain))
  ;   ((lambda-exp? proc)
...
(define (map-1 proc items)
  ((lambda (f n)  ; this lambda is defining MAP 
      ...
      (lambda (map f n) 
          (if (null? n) 
              '() 
              (cons (f (car n)) (map map f (cdr n))) )) )) proc items))
...
; (eval-1 '((lambda (x) (+ x 3)) 5))             ;进了pair？
; (car '((lambda (x) (+ x 3)) 5))                ;(car exp)         | '(lambda (x) (+ x 3))
; (cdr '((lambda (x) (+ x 3)) 5))                ;(cdr exp)         | '(5)
; (eval-1 (car '((lambda (x) (+ x 3)) 5)))       ;(eval-1 (car exp) | '(lambda (x) (+ x 3))

; (map eval-1 (cdr '((lambda (x) (+ x 3)) 5)))   ;(map eval-1 (cdr exp))  |'(5)
...
;进入apply-1
;(apply-1 proc args)
;proc: (eval-1 (car '((lambda (x) (+ x 3)) 5)))
;args: (map eval-1 (cdr '((lambda (x) (+ x 3)) 5)))

; (procedure? (eval-1 (car '((lambda (x) (+ x 3)) 5))))   ;#f
; (lambda-exp? (eval-1 (car '((lambda (x) (+ x 3)) 5))))  ;#t

; (substitute '(+ x 3) '(x) '(5) '())
; (substitute '(+ x y z) '(x y z) '(1 2 3) '())
;------------------
;进入substitute
;exp:    (caddr proc)  ;'(+ x 3)
;params: (cadr proc)   ;'(x)
;args:   (map eval-1 (cdr '((lambda (x) (+ x 3)) 5))) ;'(5)
;bound:  '()

; (caddr (eval-1 (car '((lambda (x) (+ x 3)) 5))))  ; (caddr proc) | '(+ x 3)
; (cadr (eval-1 (car '((lambda (x) (+ x 3)) 5))))   ; (cadr proc)  | '(x)
; (map eval-1 (cdr '((lambda (x) (+ x 3)) 5)))      ; args         | '(5)
;进了else

; (map (lambda (subexp) (substitute subexp params args bound)) exp)
; (substitute '+ '(x) '(5) '())
; (substitute 'x '(x) '(5) '())
; (substitute '(5) '(x) '(5) '())

; (map (lambda (subexp) (substitute subexp '(x) '(5) '())) '(+ x 3)) ; |'(+ 5 3)
; (eval-1 1)

; (map-1 first '(the rain in spain))
; (map-1 (lambda (x) (first x)) '(the rain in spain))

; (map first '(the rain in spain))
; (eval-1 '(map first '(the rain in spain)))

; (pair? '(map first '(the rain in spain))) ;#t
; (car '(map first '(the rain in spain)))   ;(car exp) | 'map
; (cdr '(map first '(the rain in spain)))   ;(cdr exp) | '(first '(the rain in spain))

; (eval-1 (car '(map first '(the rain in spain))) )  ;(eval-1 (car exp) ; #<procedure:map>

; (apply-1 map '(first (the rain in spain)))
; '(first (the rain in spain))
(define (combine-params params)
  (if (null? params)
      '()
      (cons (caar params) (combine-params (cdr params)))
  )
)

(define (combine-args args)
  (if (null? args)
      '()
      (cons (cadar args) (combine-args (cdr args)))
  )
)


(define (let params exp)
  (substitute exp (combine-params params) (combine-args params) '())
)
